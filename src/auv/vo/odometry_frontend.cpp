...
OdometryEstimate OdometryFrontend::TrackStereoFrame(const Image1b& iml, const Image1b& imr)
{
  ...
  out.npoints_detect_left = pdetector_.Detect(iml, kpl, orbl);
  out.npoints_detect_right = pdetector_.Detect(imr, kpr, orbr);

  std::vector<int> pmatches_lr;
  out.npoints_matched_stereo = StereoMatchPoints(
  ...
  if (has_prev_frame) {
    ...
    out.npoints_matched_temporal = MatchPointsNN(
  ...
  kpl_prev_.resize(out.npoints_matched_stereo);
  orbl_prev_ = cv::Mat(out.npoints_matched_stereo, orbl.cols, orbl.type());
  disp_prev_.resize(out.npoints_matched_stereo);
  ...
  out.C_0_1 = C_10;
  out.error = error;
  out.npoints_tracked = point_inlier_indices.size();
