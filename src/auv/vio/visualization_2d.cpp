                          ...
                          const std::vector<cv::KeyPoint>& untracked_cur)
{
  ...
  for (const auto& kp : untracked_ref) {
    cv::circle(img_bgr, kp.pt, 4, cv::viz::Color::red(), 1);
  }
  for (const auto& kp : untracked_cur) {
    cv::circle(img_bgr, kp.pt, 4, cv::viz::Color::blue(), 1);
  }

  // Show tracks from ref_img to cur_img.
  for (size_t i = 0; i < ref_keypoints.size(); ++i) {
    const cv::Point& px_cur = cur_keypoints.at(i).pt;
    const cv::Point& px_ref = ref_keypoints.at(i).pt;
