                                     ...
                                     const cv::Point2f& left_keypoint)
{
  ...
  const int stripe_rows = opt_.templ_rows + 2;

  const int rounded_lkp_x = round(left_keypoint.x);
  const int rounded_lkp_y = round(left_keypoint.y);

  int templ_topleft_y = rounded_lkp_y - (opt_.templ_rows - 1) / 2;  // y-component of upper left corner of template

  // Template exceeds top or bottom of the image, return no match.
  if (templ_topleft_y < 0 || (templ_topleft_y + opt_.templ_rows) >= left_rectified.rows) {
    return -1.0;
  }

  int offset_x = 0;
  int templ_topleft_x = rounded_lkp_x - (opt_.templ_cols - 1) / 2;
  ...
  if ((templ_topleft_x + opt_.templ_cols) >= left_rectified.cols) {
    if (offset_x != 0) {
      LOG(FATAL) << "offset_x exceeds left AND right bounds! This is probably a bug." << std::endl;
    }
    offset_x = (templ_topleft_x + opt_.templ_cols) - (left_rectified.cols - 1);
    templ_topleft_x -= offset_x;
  }

  // Grab the local image patch around the keypoint.
  cv::Rect template_rect = cv::Rect(templ_topleft_x, templ_topleft_y, opt_.templ_cols, opt_.templ_rows);
  ...
  int offset_stripe = 0;
  int stripe_corner_x = rounded_lkp_x + (opt_.templ_cols - 1) / 2 - opt_.max_disp;
  if (stripe_corner_x + opt_.max_disp > right_rectified.cols - 1) {
    offset_stripe = (stripe_corner_x + opt_.max_disp) - (right_rectified.cols - 1);
  ...
  cv::Rect stripe_rect = cv::Rect(stripe_corner_x, stripe_corner_y, opt_.max_disp, stripe_rows);
  ...
  matchLoc.x += stripe_corner_x + (opt_.templ_cols - 1) / 2 + offset_x;
  matchLoc.y += stripe_corner_y + (opt_.templ_rows - 1) / 2;
  cv::Point2f match_px(matchLoc.x, matchLoc.y);

  // Refine keypoint with subpixel accuracy.
  if (opt_.subpixel_refinement) {
  ...
  const bool has_good_matching_score = minVal < opt_.max_matching_cost;
