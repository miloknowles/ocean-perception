...
TEST(StereoCameraTest, TestProject)
{
  ...
  const Vector2d pl = stereo_cam.LeftIntrinsics().Project(Vector3d(0, 0, 10));
  ASSERT_EQ(376.0, pl.x());
  ASSERT_EQ(240.0, pl.y());

  const Vector2d pl2 = stereo_cam.LeftIntrinsics().Project(Vector3d(1, 2, 3));
  EXPECT_NEAR(376.0 + 415.876509 * 1 / 3, pl2.x(), 1e-3);
  EXPECT_NEAR(240.0 + 415.876509 * 2 / 3, pl2.y(), 1e-3);

  const Vector3d Pr = Vector3d(-3, 4, 10);
  const Vector2d pr = stereo_cam.RightIntrinsics().Project(Vector3d(-3, 4, 10));
  ASSERT_EQ(Pr, stereo_cam.RightIntrinsics().Backproject(pr, 10));
}
